    ...
    if z.status_code==200: #check if API call is a success
        ...
        for k in std_dict.keys():
            if count == 1:
                ...
                phrase += "%s='%s'" % (k, str(std_dict[k]))
        phrase = "UPDATE accounts SET %s WHERE user_id = '%s' and provider_id = '%s' and r_token= '%s'" % (phrase,TL_auth.uid,provider_id,refresh_token)
        phrase = phrase.strip()
        print(phrase)
...
def new_token(provider_id,access_code):
    ...
    c.execute("select * from accounts where user_id=? and provider_id=?", (TL_auth.uid, provider_id))
    if c.fetchone()!=None:
    ...
    # the user does not exist so call Truelayer and exchange code for access token
        ...
        payload = {'grant_type': 'authorization_code', 'client_id': TL_auth.client_id, \
                   'client_secret': TL_auth.client_secret, 'redirect_uri': TL_auth.red_url, 'code': access_code}
        r_lasttime=datetime.now()
        z=requests.post(TL_auth.token_url, data=payload) # call truelayer to get the token and set the call time
        ...
        if z.status_code==200: #check if API call is a success
            ...
            for k in temp_dict.keys():
                if k in map_dict.keys():
                    ...
                    extra_data[k]=str(temp_dict[k])
            std_dict['r_lasttime']=str(r_lasttime)
            std_dict['user_id']=str(TL_auth.uid)
...
def access_token(provider_id):
    ...
    c.execute("select a_token from accounts where user_id=? and provider_id=?", (TL_auth.uid,provider_id))
    token=c.fetchone()[0]
    return(token)
